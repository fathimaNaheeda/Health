...
<body>
	<div class="panel panel-success">
		...
		<div class="panel-heading">Laboraties Management</div>
		<div class="col-md-6 col-sm-12 btn-margin-bottom action-buttons">
			<div class="btn-group" id="divAddNew">
				<a title="Add new" class="btn btn-block btn-danger btn-flat"
					href="./getLabRegForm"><span class="hide-on-mobile">Add
		...
		<table class="table">
			...
			<c:forEach items="${uiListLaboraties}" var="labBean">
				<!-- equals code:for(LaboratiesBean LabBean:uiLabList) by using for-each loop  -->
				<tr>
					<td>${labBean.labId}</td>
					<td>${labBean.firstName}</td>
					<td>${labBean.lastName}</td>

					<td>${labBean.email}</td>
					<td>${labBean.phone}</td>
					<td>${labBean.status}</td>

					<td><a href="./editLaboraties?labId=${labBean.labId}"><div
								class="glyphicon glyphicon-edit"></div></a></td>
					<td><a href="./deleteLaboraties?labId=${labBean.labId}">
		...
		<%-- <nav>
  <ul class="pagination">
   <li><a href="./getAllLabByPaging?currentPage=${loop.count-1}" aria-label="Previous"> <span
     aria-hidden="true">&laquo;</span>
     </a></li>
      <c:forEach
      items="${pageBarList}" var="hospBean" varStatus="loop">
      <li><a href="./getAllLabsByPaging?currentPage=${loop.count}">${loop.count}</a></li>
     </c:forEach>
     <li><a href="./getAllLabsByPaging?currentPage=${loop.count+1}" aria-label="Next"> <span
